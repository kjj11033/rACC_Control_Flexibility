...
def wiener_like_addm_ics(np.ndarray[double, ndim=1] x,
    ...
    for j in range(N):
        ...
        pe=abs(conflicts-qs_pre)
            
        qs = qs_pre + alfa*(conflicts-qs_pre)
        
        om_trans = (2.718281828459**(om+ics_om*ics_trial*pe)) / (1 + 2.718281828459**(om+ics_om*ics_trial*pe))

        if conflicts==1:
            p = full_pdf(xs, (v+ics_v*ics_trial)*((1-om_trans)*qs_pre+om_trans), sv, a, z,
                         sz, t, st, err, n_st, n_sz, use_adaptive, simps_err)
        elif conflicts==0:            
            p = full_pdf(xs, (v+ics_v*ics_trial)*((1-om_trans)*(1-qs_pre)+om_trans), sv, a, z,
                         sz, t, st, err, n_st, n_sz, use_adaptive, simps_err)        
        qs_pre = qs           
